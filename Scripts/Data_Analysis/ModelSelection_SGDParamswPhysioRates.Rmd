...
FULL_RespoNEC_allParams$new_colonynumber <- as.character(FULL_RespoNEC_allParams$new_colonynumber)
...
MS_V_NP_nonlinear <- RespoR_Normalized_Full_ExcelUpdates_V_NP %>%
  ...
  mutate(fit = map(data, ~lmer(Response_Variable~ poly(Predictor_Values, degree=2)  + (1|new_colonynumber), data = .))) #create models
MS_V_NP_nonlinear
...
MS_C_NEC_linear <- NEC_wnuts_C %>%
  nest(data = -c(Predictor_Variable)) %>% # nest all the data measurement type
   mutate(fit = map(data, ~lmer(Response_Variable~ poly(Predictor_Values, degree=2)  + (1|new_colonynumber), data = .))) #create models ### KEEP NEW COLONY AS RANDOM 
MS_C_NEC_linear
...
MS_C_NEC_linear_fitresults_cleannames <- MS_C_NEC_linear_fitresults %>%
  ...
  mutate(delAIC = AIC - min(AIC)) %>%
  arrange(delAIC)
## result: pH is lowest AIC, with sig p-val - rest are more than 2 AIC 
