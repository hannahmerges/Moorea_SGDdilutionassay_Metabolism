...
-  **Figure 2:** Results of raw groundwater samples between Cabral and Varari compared to ambient seawater for six parameters (salinity, pH, temperature, TA, nitrates, phosphates, and silicate) - (see script: "Plots_Based_on_Model_Selection" for code, line 1505)
...
-  **Supplementary Figure 1:** Correlation matrix showing differences between correlation of silicate to other parameters at Cabral vs Varari (see script: "Plots_Based_on_Model_Selection" for code, line 1373)
...
variance_plot <- R_var %>%
  ...
  mutate(grp = factor(grp, levels = c("Site","Residual","Putative Genotype","Date"))) %>% # put the levels in an order I want for plotting
  ...
  scale_fill_manual(values=c("deepskyblue2", "gray", "coral3", "burlywood"))
...
standardized_effects <- all_std %>%
  mutate(term = case_when(term == "log_silicate"~ "Log Silicate",
                          term == "I(log_silicate^2)"~ "Quadratic Log Silicate")) %>% 
