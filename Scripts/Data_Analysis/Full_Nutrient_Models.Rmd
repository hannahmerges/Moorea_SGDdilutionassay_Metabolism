...
#Timeline Updates
at some point: go back to thoroughly check the correct code for rates in the Respo_Normalized_Full csv !!!!!!!
also fixed NEC rates around May 28th because there was an error in the code 
RespoR_Normalized_Full_ExcelUpdates = HAS NUTRIENT DATA FOR GP 
...
- successfully joined dataframes!! on June 1st --> important note: NEC still included Cabral 19 and 20, but that was previously deleted in Respo data --> 
- SHOULD GO BACK AT SOME POINT TO DOUBLE CHECK IF THAT IS STILL APPROPRIATE 

- dropping NAs in some of the umol.cm2.hr data -- is that okay? 
...
```{r}
###  make new dataset 
Nutrient_Models_Varari_NP <- RespoR_Normalized_Full_ExcelUpdates %>% 
  filter(site=="Varari") %>% 
  drop_na(NN_umolL) %>% 
  filter(P_R=="NP") %>% 
  filter(!is.infinite(umol.cm2.hr)) %>% 
...
```{r}
Nutrient_Models_Varari_GP <- RespoR_Normalized_Full_ExcelUpdates %>% 
  filter(site=="Varari") %>% 
  drop_na(NN_umolL) %>% 
  filter(P_R=="GP") %>% 
  filter(!is.infinite(umol.cm2.hr)) %>% 
...
```{r}
Nutrient_Models_Varari_NEC <- RespoR_Normalized_Full_ExcelUpdates %>% 
  filter(site=="Varari") %>% 
  drop_na(NN_umolL) %>% 
  filter(P_R=="C") %>% 
  filter(!is.infinite(umol.cm2.hr)) %>% 
...
### Cabral R plots 
```{r}
Nutrient_Models_Cabral_R <- RespoR_Normalized_Full_ExcelUpdates %>% 
  filter(site=="Cabral") %>% 
  drop_na(NN_umolL) %>% 
  filter(P_R=="R") %>% 
  filter(!is.infinite(umol.cm2.hr)) %>% 
