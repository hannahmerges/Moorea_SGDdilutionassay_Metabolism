...
pHSlope<-pHcalib %>%
  ...
  right_join(.,BioData) %>% # join with the pH sample data
  mutate(mVTRis = temp_C*Ttris + `(Intercept)`) %>% 
  drop_na(TrisCalDate) %>% 
  mutate(new_pH = pH(Ex=mV,Etris=mVTRis,S=salinity,T=temp_C)) 

pHSlope_edit <- pHSlope %>% 
  select(!c("TrisCalDate":"Ttris", "SGD_dil":"mVTRis"))

write_csv(pHSlope_edit, here("Data", "pHSlope.csv"))

```

## use Respo R csv and tidy data 
```{r}
RespoR2 <- RespoR %>%
...
RespoR_Normalized_blankperday_joined <- RespoR_Normalized_blankperdaynoCabral %>% 
  ...
  #ungroup() %>%
  dplyr::select(SGD_number, light_dark, blank.rate, site, date, sample_ID, SGD_dil, new_colonynumber, SA_cm2, salinity, temp_C, phosphate_umolL, silicate_umolL, NN_umolL, ammonia_umolL, umol.cm2.hr, umol.sec, umol.sec.corr, umol.cm2.hr_uncorr)

#join with Tris pH data 

TrispH_RespoRNormalized_perdayBlanks <- RespoR_Normalized_blankperday_joined %>% 
  left_join(pHSlope_edit)
...
RespoR_Normalized_Full <- RespoR_Normalized2 %>% 
  dplyr::select(new_colonynumber, new_pH, SGD_number, site, salinity, date, light_dark, P_R, umol.cm2.hr, umol.cm2.hr_uncorr, phosphate_umolL, silicate_umolL, NN_umolL, ammonia_umolL) %>%
...
write_csv(RespoR_Normalized_Full , here("Data","RespoFiles","Respo_RNormalized_Full.csv"))  
