...
RespoR_Normalized_Full <- RespoR_Normalized2 %>% 
  dplyr::select(colony_number, new_pH, SGD_number, site, salinity, light_dark, P_R, mmol.cm2.hr, mmol.cm2.hr_uncorr) %>% 
...
ggsave(here("Outputs", "RespoOutput","GPRates_Salinity.jpg"), 
       width = 10, height = 10)
...
ggsave(here("Outputs", "RespoOutput","RRates_Salinity.jpg"), 
       width = 10, height = 10)
...
RatesPlot_R_EnvParameters <- RespoR_Normalized_Full %>%
  ...
  geom_point()+ 
  labs(title="GP by pH")
ggsave(here("Outputs", "RespoOutput","GP_pH.jpg"), 
       width = 10, height = 10)
...
ggsave(here("Outputs", "RespoOutput","R_pH.jpg"), 
       width = 10, height = 10)
...
RData <- RespoR_Normalized_Full %>% filter(P_R == "R")
model3 <- lm(data = RData, mmol.cm2.hr ~ new_pH)
anova(model3)
HSD.test(model3, "pH", console=TRUE)

GPData <- RespoR_Normalized_Full %>% filter(P_R == "GP")
model1 <- lm(data = GPData, mmol.cm2.hr ~ new_pH)
anova(model1)
