...
RespoR_Normalized_blankperday_joined <- RespoR_Normalized_blankperday %>% 
  ...
  #ungroup() %>%
  dplyr::select(date, sample_ID, light_dark, SA_cm2, umol.cm2.hr, umol.sec, blank.rate, Temp.C, new_colonynumber, salinity, pH, site, SGD_number, umol.sec.corr, umol.cm2.hr_uncorr)
...
graphdata <- as.data.frame(emmeans(model2_SGDbyR, "SGD_number"))
...
### using a new R package to calculate R2 values 
